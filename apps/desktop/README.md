...
Electron Builder stores the runtime in `app.asar` and leaves only native modules and executables in `app.asar.unpacked`. The child process installs a resolver hook before the CLI starts; ordinary module resolution remains first, while unresolved in-box profile plugins fall back to the runtime inside ASAR. This avoids installing tens of thousands of dependency files without changing profile-local plugin precedence. Production releases should also be code-signed because real-time malware scanners can significantly delay unsigned installer creation and installation.
