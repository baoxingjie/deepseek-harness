...
The `cordis` preset ("creation mode") loses its entire skill catalog in a packaged build. That preset adds its own skill root under the preset directory, which packaging places inside the ASAR archive; Electron patches `fs.readdir` for archive paths but not `fs.watch`, so the filesystem skill provider's watcher startup fails with `ENOTDIR`. The provider then reports its candidates as an incomplete observation, and the skill tool withholds the whole merged catalog rather than advertising a possibly stale one — including every skill from every other provider. `asarUnpack` alone does not help, because `fs.watch` is not redirected to the unpacked copy either. Only the `cordis` preset configures such a root, so the shipped default (`standard`) and the other presets are unaffected.
