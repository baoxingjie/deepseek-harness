...
`apps/desktop` is an Electron shell over the shipped Web profile. Its main process launches the deployed `dsh` CLI through Electron's Node mode, binds the server to `127.0.0.1` on an OS-assigned port, and waits for the existing `dsh web:` readiness line before showing a window. The main module schedules this work after `app.whenReady()` resolves without awaiting readiness during module evaluation, because Electron emits readiness only after main-module evaluation completes. The renderer uses the existing HTTP, SSE, static-file, plugin, and permission paths.
