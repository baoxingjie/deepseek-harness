...
1. 打开 **http://127.0.0.1:3082/** → 未登录时工作台前会挡一层登录卡片（手机号 + 图形验证码 + 短信验证码）。已登录则直接进工作台，登录卡片不出现
2. 登录成功后插件自动完成：
...
- 注册的路由：`/api/uniclaw/login/{captcha,sendCode,smsLogin}`、`/api/uniclaw/my-plan`、`/api/uniclaw/status`（均为元景网关代理）、`/uniclaw`（一期登录页）、`/api/uniclaw/diagnostics/skills`（只读技能目录诊断）
...
- 内置技能的落盘位置：源在包内 `skills/`，但 Electron 打包后它整个在 `app.asar` 里。Electron 给主进程打过 `fs` 补丁，所以插件读 SKILL.md 没问题、目录也列得出来；而技能自己的 `scripts/*.py` 是由 shell、python 这些**子进程**执行的，它们走未打补丁的系统调用，看到的 asar 只是一个普通文件——14 个内置技能里 11 个带脚本、9 个在 SKILL.md 里用相对路径调它们，全都会 ENOENT。所以路径一旦落在 `.asar` 段内，provider 就先把整棵树materialize 到 `<dshHome>/uniclaw-builtin-skills/<版本>-<指纹>/` 再对外提供；开发态路径本来就是真实目录，原地提供，不做无谓拷贝
- materialize 的替换语义：目录名里的指纹是对每个文件的相对路径与大小求的 sha256 前 12 位，所以载荷一变必然换目录——包含 rc 阶段复用同一版本号重出包的情况。拷贝先落在同级的 `.incoming-<pid>`，完成后 `rename` 就位，中断的运行只会留下暂存目录，不会留下一个会被误判为完整的目标目录；目标就位后清掉其余指纹的目录，所以升级/重装是替换而非堆积。内置技能与用户安装的技能全程分开：用户装的仍只在 `<dshHome>/skills/`，内置的走独立目录并以 `source: 'bundled'` / `provider: 'uniclaw-bundled'` 标识，UI 上仍是「内置」且不给卸载按钮
...
- 技能目录诊断：[src/diagnostics.ts](src/diagnostics.ts)，`GET /api/uniclaw/diagnostics/skills`，只读。报告合并后的目录（每条的 `provider` / `source` / `modelInvocable` / `resource`）、`byProvider` 计数、`complete`，以及内置包的扫描根、可读性与停用名单。`?cwd=` 指定工作区，`?probe=<name>` 额外走一遍 `ctx.skills.get()` 并回报字节数与 `path`——这一步才能区分「列出来了」和「读得出来」（ASAR 内路径失效是后者）。存在的理由：目录只经由 system prompt 到模型、经由 settings RPC 到浏览器，命令行不可见；而「装了 N 个包」不等于「模型看得见 N 个技能」（注册表会丢弃校验失败的候选、按 rank 遮蔽重名），所以数目录数量不构成证据。视图是全局层，运行中的 agent 还叠加它自己的 preset 层，故 agent 目录是本报告的超集——这里没有的，哪里都没有
...
下一步：套餐/用量单开一页；再在外面套 Electron 壳（主进程 spawn `dsh web` + 窗口加载本地地址）。
