...
```bash
# 将目录中所有 PPTX 文件转换为图片
python <技能根>/scripts/ppt_to_pic.py --ppt-dir ./ppt --output-dir ./images

# 将单个 PPTX 文件转换为图片
python <技能根>/scripts/ppt_to_pic.py --file presentation.pptx --output ./images
...
```bash
# 描述单张图片
python <技能根>/scripts/vision_qwen.py --image path/to/image.png --prompt "描述这张幻灯片"

# 批量描述多张图片（每批最多 5 张）
python <技能根>/scripts/vision_qwen.py --images img1.png img2.png img3.png
```
...
```bash
python <技能根>/scripts/office/soffice.py --headless --convert-to pdf output.pptx
