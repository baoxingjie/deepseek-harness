...
A chart lives in a `<figure>` (or card `<div>`) that owns responsive sizing, the
title/caption, and the **table-view toggle — the accessibility twin of every
chart**, the WCAG-clean way to read any value that color alone encodes.

**Any fixed height includes the x-axis band** (plot height + axis labels), so the
card never develops a nested vertical scroll. Prefer letting the container grow
with its content over pinning a height at all.
