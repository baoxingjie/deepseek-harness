...
**CJK.** When any axis label, legend entry, or title is Chinese, the stack above
falls through to a system default that varies by platform. Use the CJK-augmented
stack instead — same system-sans intent, with explicit Chinese faces:
`system-ui, "PingFang SC", "Microsoft YaHei", "Noto Sans SC", sans-serif`.
In ECharts this goes on `textStyle.fontFamily` at the root of the config.
