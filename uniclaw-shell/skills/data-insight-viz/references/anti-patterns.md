...
**❌ Shipping a config with no `color` field.** ECharts then falls back to its own
defaults (`#5470c6`, `#91cc75`, `#fac858`, …), whose adjacent hues collapse into
one another under red/green CVD. `validate_echarts.py` does **not** check color, so
nothing catches this — an omitted `color` array is a silent failure.
✅ Every config carries an explicit `color` array from `palette.md`, mode-correct.

**❌ Recoloring by cycling: 9+ series, letting ECharts wrap the palette.**
✅ Eight slots is the ceiling. Fold the tail into "其他", or split into small
multiples. See the series-count ladder in `choosing-a-form.md`.

**❌ A pie with more than 5 slices, or a donut used to compare close values.**
✅ Horizontal stacked bar, or a table. Part-to-whole at a glance only.

**❌ Hand-tuning `grid.left/right/top/bottom` to fix clipped labels.** Guessed
margins break at the next data set.
✅ `"grid": {"containLabel": true}` and nothing else, unless you have verified
exact values.

**❌ A JavaScript function as a `formatter`.** The config must be valid JSON; a
function makes it unparseable.
✅ String templates — `"formatter": "{b}: {c}"`.

**❌ Status semantics and categorical slots mixed in one chart** — e.g. an
"error rate" series wearing slot-3 yellow while a neighboring series wears the
status warning token.
✅ A series that *means* good/bad wears status tokens; a series that is merely
"the 4th series" wears categorical. Never both in the same chart.
