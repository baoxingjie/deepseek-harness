...
from __future__ import annotations  # PEP 604 annotations (`str | None`) on Python 3.9
...
def _pip_install(*packages: str) -> None:
    """Install packages, then make them importable in THIS process.

    pip may create the user site-packages directory only now; it was absent when
    the interpreter built sys.path, so a plain `import` right after would still
    fail and the first run would always crash. site.main() re-adds it.
    """
    os.system(f"{sys.executable} -m pip install {' '.join(packages)} -q")
    site.main()
