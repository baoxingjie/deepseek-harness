...
> **布局参考**：[layouts.md](layouts.md) 提供 14 种现成布局模板，直接复制使用
> **主题系统**：使用 `require("./theme.js")` 引入主题，所有颜色/字体/样式通过 THEME 变量调用
...
const THEME = require("./theme.js");  // 由 theme_generator.js 从 <技能根>/themes/*.json 生成
...
slide.addText("Title", THEME.titleStyle({ x: 0.5, y: 0.5, w: 9, h: 1.5 }));
slide.addText("Body text", THEME.bodyStyle({ x: 0.5, y: 2.5, w: 9, h: 2, color: THEME.colors.text_on_dark }));

// 样式工厂每次返回新对象，可安全使用 spread 覆盖：
