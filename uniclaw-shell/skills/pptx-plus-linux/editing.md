...
1. **分析现有幻灯片**：
   ```bash
   python <技能根>/scripts/thumbnail.py template.pptx
...
3. **解包**：`python <技能根>/scripts/office/unpack.py template.pptx unpacked/`
...
6. **清理**：`python <技能根>/scripts/clean.py unpacked/`

7. **打包**：`python <技能根>/scripts/office/pack.py unpacked/ output.pptx --original template.pptx`
...
```bash
python <技能根>/scripts/office/unpack.py input.pptx unpacked/
```
...
```bash
python <技能根>/scripts/add_slide.py unpacked/ slide2.xml      # 复制幻灯片
python <技能根>/scripts/add_slide.py unpacked/ slideLayout2.xml # 从布局创建
```
...
```bash
python <技能根>/scripts/clean.py unpacked/
```
...
```bash
python <技能根>/scripts/office/pack.py unpacked/ output.pptx --original input.pptx
```
...
```bash
python <技能根>/scripts/thumbnail.py input.pptx [output_prefix] [--cols N]
```
